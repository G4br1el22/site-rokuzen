...
const defaultAvatarSrc = ""; // Defina o caminho para um avatar padrão se necessário.
...
const uploadFotoInput = document.getElementById("uploadFotoInput"); // NOVO ID
const removerFotoBtn = document.getElementById("removerFotoBtn"); // NOVO BOTÃO
...
function selecionarUsuario(id) {
    ...
    // Reseta o input de arquivo (para garantir que uma foto antiga não seja enviada novamente se o usuário selecionar outro arquivo)
    uploadFotoInput.value = "";
}


// NOVO EVENTO: Carregar nova foto
uploadFotoInput.addEventListener("change", e => {
    ...
    if (file) {
        ...
        reader.onload = () => {
            ...
            // A foto é salva em preview.src (e será persistida ao clicar em Salvar)
        };
        reader.readAsDataURL(file);
    }
});

// NOVO EVENTO: Remover foto
removerFotoBtn.addEventListener("click", () => {
    if (!usuarioSelecionado) return;
    
    // Define a foto como o avatar padrão (vazio)
...
// Lógica para o dropdown do avatar
...
form.addEventListener("submit", e => {
    ...
    usuarioSelecionado.foto = preview.src; // Salva o Base64 atual (se foi carregado ou removido)
